...
- 桌面客户端已经进入 Electron/Vue 本地 runtime client 方向，负责聊天、审批、Markdown 渲染、历史会话和会话操作。
...
- 支持 `conversation_title_request`，当前标题生成策略为根据首条 user 消息调用配置模型生成短标题，返回前清理与截断，来源标记为 `model-first-user`。
